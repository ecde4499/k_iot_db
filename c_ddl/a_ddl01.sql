...
-- DB(스키마, Schema) 생성 --
### create: 데이터베잇그 생성, 데이터를 저장하고 관리하는 첫 단계
# 기본 형태
...
CREATE DATABASE example2; -- Can't create database 'example2'; database exists > 이미 존재해서 생성 불가
# >> 데이터베이스 이름은 중복될 수 없음
# >> 같은 이름의 DB 두 번 생성 불가
...
# cf) ci (case-incensitive): 대소문자 구분하지 않음 ('abc' = 'ABC')
