...
# 2) 속성
-- 엔티티의 세부정보, 설명 항목
-- 일반 속성(이름, 주소, 전화번호 등), 기본기(PK: 회원 ID, 학번 등), 외래키(FK: 주문테이블의 고객 ID)
-- 타원형 (속성명은 원 안에 명시)
...
# cf) 관계 유형
# 1:1(일대일) - 한 엔티티가 다른 하나와만 관계 (사람 - 여관)
# 1:n(일대다) - 한 엔티티가 여러 개와 관계 (교수 - 강의, 학생 - 수장, 게시글 - 댓글)
# N:N(다대다) - 여러 엔티티가 서로 다수와 관계(학생 - 과목: 중간테이블 필요)

##### ERD 표현 규칙 #####
# 1) 부모 PK를 자식이 FK로 가짐
#	> 실선 관계: 자식이 부모의 존재에 종속됨 (부모가 있어야만 자식이 존재 - PK + FK)
#	> 점선 관계: 자식이 독립적인 존재 가낭(부모가 없어도 자식 생성 가능 - FK)

# 2) 예시 1
# 학생과 수강 내역
# 학생 테이블) 부모 에티블 = 학번 PK, 학생명 ...등
# 수강 내역 테이블) 자식 테이블 - 수강 IF PK, 학번 FK, 강의명 ... 등

# >> 관계: 1명의 학생은 여러 개의 수강 내역을 가질 수 있음 (1:N 관계)

# 3) 예시 2
# 주문과 주문 상세 내역
# 주문 테이블) 부모 - 주문 ID PK, 주문 일시 ... 등
...
# 4. 생성하고자 하는 스키마(Datavbase) 선택 > Next
# 5. Execute 실행 - 생성
...
# 주문(Order)은 0개 이상의 리뷰(Review)를 가질 수 있음 (Ordet 0< Review) - 0:N
