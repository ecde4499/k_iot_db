...
# : 정규화의 두 번째 단계
# - 1NF(제1정규화)를 만족하면서 '모든 비기본 속성(일반 컬럼)'이 기본키에 완전이 종속된 상태
#		>> 완전 함수적 종속 상태
...
#		- 2NF 위반
#		Ex) 기본키가 (A, B)일 때, D(비기존)가 A만으로 결정 >> 부분 종속
...
create table departments(
	departmenet_id bigint primary key, -- 기본키
    # 1NF 상태: 각 컬럼에 원자값만 존재
    # 기본키(departmenet_id)
    # > 부서의 기본키를 제외한 부서 위치, 부서장 ID가 departmenet_id 완전 종속 O
    location varchar(8), -- 부서 위치
    supervisor_id bigint -- 부서장 ID 
    
    # 직원 연봉, 전체 부서의 개수 - 2NF를 위반한 속성
);


create table employees (
	employee_id bigint primary key, -- 기본키
    departmenet_id bigint, -- 직원이 속한 부서 ID
	
    foreign key (departmenet_id) references departments(departmenet_id)
);
