...
/*
    ...
    2. FROM 테이블명: 어떤 테이ㅏ블에서 데이터를 가져올 지 결정
    ---
    ...
    cf) DB 내부 실제 실행 순서
    FROM 테이블 > JOIN(추가 테이블 데이터 가져오기) > WHERE(필터링) > GROUP BY(그룹화)
		> HAVING(조건) > SELECT(컬럼 추출) > ORDER BY(정렬) > LIMIT(제한)
*/

USE korea_db;

## 1. 기본 조회 ##
# : SELECT 컬럼명 FROM `데이터베이스명`.`테이블명`; - 데이터명은 필수 X(오류 방지용)
...
# cf) null은 '값이 없음'을 나타냄: 그 어떤 값과도 비교하거나 연산이 불가
...
# 1개의 임의의 문자 + u + 0개 이상의 임의의 문자 허용 검색
# >> 이름이 두 번째 글자가 u인 모든 회원 조회: _u%
SELECT * FROM `members`
WHERE
	name LIKE '_u%';
