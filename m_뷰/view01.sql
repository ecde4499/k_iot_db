...
# - 필요한 데이터만 보는 용도
# - 기본 테이블 변경 시 자동 반영
# >> CREATE VIEW 뷰명 AS SELECTE문(TABLE)
...
# >> 뷰는 테이블처럼 보이지만, 내부적으로는 SELECT문으로 작동
...
# Sprin Boot는 주로 JPA 또는 MyBatis를 통해 DB와 연동
# - 이 때, 뷰튼 일반 테이블 처럼 인식

# cf) 뷰(View) VS 반정규화
# 뷰: 실제 데이터를 복제하지 않고 조인 결과를 미리 정의 ('마치 테이블처럼')
# 반정규화: 정규화 된 테이블을 의도적으로 중복하거나 조인 없이 바로 조회할 수 있도록 재구성 - 성능 최적화
