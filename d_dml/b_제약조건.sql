...
CREATE TABLE `orders` (
    ...
    # 외래 키 지정 방법
    # foreign key (참조컬럼, 현 테이블의 컬럼) references 기본테이블(기본컬럼, pk컬럼)
...
#  a foreign key constraint fails (`example`.`orders`, CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`member_id`) 
