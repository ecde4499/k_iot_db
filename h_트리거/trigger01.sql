...
# 1. 트리거 사용 목적
-- 일정 작업 자동화: 일련의 동작을 같이 처리 (회권 가입 시 가입 로그 자동 기록)
...
# cf) DML 이벤트를 감지(INSERT, UPDATE, DELETE) - SELECT는 감지 X
...
# cf) 원자성(Atomicitiy): 모두 성공 또는 모두 실패 (하나라도 실패시 모두 롤백)
